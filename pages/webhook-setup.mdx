...
**Webhook triggers**: choose which events should trigger webhooks, and which fields should trigger the `application_updated` event. You can uncheck fields that aren't important to you (for example, `broker.suretyBonds`). This means updates to this field won't trigger a webhook or be mentioned in `fieldsUpdated`.
