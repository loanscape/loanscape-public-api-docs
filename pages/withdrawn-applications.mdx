...
Withdrawn applications will be returned by our [Fetch all applications](/api-reference/endpoint/get_applications) and [Fetch an applications](/api-reference/endpoint/get_application) endpoints, but with limited fields - only `applicationId`, `status` ("withdrawn"), `dateCreated` and `dateStatusUpdated`.
